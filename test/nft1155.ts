...
describe("NFT1155", function () {
  it("name, symbol, mint, ownerOf, balanceOf, tokenURI, tokenURI url, tokenURI json", async function () {
    const uri = "NFT1155";
    ...
    await nft.mint(user.address, amount, metadata);

    const id = await nft.lastTokenId();
    let idNoExists = ethers.BigNumber.from(Number(id) + 1).toString();

    expect(await nft.balanceOf(user.address, id)).to.equal(amount);
    ...
    await nft.mint(user.address, amount, metadata2);
    const id2 = await nft.lastTokenId();
    expect(isJson(await nft.tokenURI(id2))).to.equal(true);

    await expect(nft.connect(user2).mint(user2.address, amount, metadata)).to.be.revertedWith('Restricted to members.');
